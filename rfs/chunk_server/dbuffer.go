...
func NewDBufferId(id common.ChunkHandle) common.BufferId {
	now := time.Now()
	tstamp := now.Nanosecond() + now.Minute()*60*1000 + now.Second()*60 //  random nanosec + minute + second time allocation
...
func (dbuf *dbuffer) Set(bufId common.BufferId, data []byte) {
	dbuf.mu.Lock()
	defer dbuf.mu.Unlock()
	dbuf.buffer[bufId] = &bufferedItem{data: data, expire: time.Now().Add(dbuf.expire)}
}
