...
func TestAppendDataFromChunckServer(t *testing.T) {
	...
	client := NewClient(common.ServerAddr(addr), 30*time.Millisecond)
	defer client.Close()
	handle, err := client.GetChunkHandle("/video/captain-america-3", common.ChunkIndex(0))
	if err != nil {
		log.Err(err).Stack().Msg(err.Error())
		return
	}
	log.Info().Msg(fmt.Sprintf("Got a new handle : %v", handle))
	err = client.Write("/video/captain-america-3", common.Offset(0), []byte("byte is done\n"))
	if err != nil {
		log.Err(err).Stack().Msg(err.Error())
		return
	}
	...
	data := `### Captain America: The Winter Soldier - A Modern Marvel Masterpiece
	**"Captain America: The Winter Soldier"** is a pivotal film in the Marvel Cinematic Universe (MCU), 
	...
	_, err = client.Append("/video/captain-america-3", []byte(data))
	if err != nil {
		log.Err(err).Stack().Msg(err.Error())
		return
	}
